...
from detectron2.evaluation import COCOEvaluator, DatasetEvaluators, inference_on_dataset, inference_context
from detectron2.utils.logger import setup_logger, log_every_n_seconds
from detectron2.utils.comm import get_world_size, is_main_process

from data.dataset_mapper import B2SDatasetMapper
...
logger = setup_logger(name="LoTSS-GRG-detect.test_racun", termcolor="magenta")
...
def process_with_evaluator(
    model,
    data_loader,
    evaluator: Union[B2SEvaluator, B2SMultiClassEvaluator],
    callbacks=None,
):
	"""
...
def main():
	...
	MULTICLASS_FLAG = cfg.MODEL.VALIDITY_HEAD.TWO_CLASSES
	...
	membership_threshold = np.arange(0.0, 1.1, 0.1) 
	validity_threshold = None
	if not MULTICLASS_FLAG:
		# Threshold for Validity Head - 11 thresholds from 0.0 to 1.0 inclusive
		validity_threshold = np.arange(0.0, 1.1, 0.1)
	...
	test_loader = build_detection_test_loader(
		cfg,
		test_dataset_name,
		mapper=B2SDatasetMapper(cfg, is_train=False),
	)
